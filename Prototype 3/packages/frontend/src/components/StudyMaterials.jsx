...
import { Download, ChevronDown, Plus, Edit3, Trash2, RefreshCw } from 'lucide-react';
...
export default function StudyMaterials({ currentUser, setActiveTab, initialBranch, initialSemester }) {
  ...
  return (
    <div className="m3-screen study-materials-dashboard">
                    ...
                    {groupedMaterials[subjectName].map((course) => {
                      ...
                      return (
                        <article
                          ...
                          className="m3-surface-card flex items-center justify-between gap-3 shrink-0"
                        >
      ...
      {/* Slide-up Document Form Bottom Sheet Modal */}
      <AnimatePresence>
        {showModal && (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md z-[99999] flex items-end justify-center" onClick={() => setShowModal(false)}>
            <motion.div
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              exit={{ y: '100%' }}
